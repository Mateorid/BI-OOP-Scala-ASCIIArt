...
class ImageTest extends FunSuite {
  ...
  val grid1x1 = new PixelGrid[Pixel](
    Seq[Seq[Pixel]](Seq[Pixel](CharPixel('X')))
  )
  ...
  test("3x3 getters") {
    ...
    assert(img.width == 3)
    assert(img.height == 3)
    assert(img.getPixel(0, 0) == CharPixel('A'))
    assert(img.getPixel(0, 1) == CharPixel('B'))
    assert(img.getPixel(0, 2) == CharPixel('C'))
    assert(img.getPixel(1, 0) == GSPixel(1))
    assert(img.getPixel(1, 1) == GSPixel(2))
    assert(img.getPixel(1, 2) == GSPixel(3))
    assert(img.getPixel(2, 0) == CharPixel('A'))
    assert(img.getPixel(2, 1) == CharPixel('B'))
    assert(img.getPixel(2, 2) == CharPixel('C'))
  ...
  test("equaling equals 1x1") {
    ...
    val alt1x1 = new Image[Pixel](new PixelGrid[Pixel](Seq[Seq[Pixel]](Seq[Pixel](CharPixel('X')))))
